  ...
  <div class="flex flex-col items-center space-y-2">
    ...
    <div class="flex flex-row items-center">
      {#each action.inputItems as input}
        <ItemDisplay size="small" item="{gameData.itemDetailMap[input.hrid]}" amount="{input.amount}"></ItemDisplay>
      {/each}
      <Icon icon="arrow-right" />
      {#each action.outputItems as output}
        <ItemDisplay size="small" item="{gameData.itemDetailMap[output.hrid]}" amount="{output.amount}"></ItemDisplay>
      {/each}
    </div>
